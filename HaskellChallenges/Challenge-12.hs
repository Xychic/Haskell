...
decodeSpecial :: Ord a => [ListItem a] -> [a]
decodeSpecial [] = []
decodeSpecial (x:xs) = decodeSingle x ++ decodeSpecial xs
    where 
        decodeSingle (Single a) = [a]
        decodeSingle (Multiple n a) = replicate n a



list = encodeSpecial "aaaabccaadeeee"

main :: IO()
main = do
    putStrLn (show list)
    putStrLn (show (decodeSpecial list))
